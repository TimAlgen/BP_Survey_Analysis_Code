...
# Cleaning & standardizing Demographics inputs & converting them to numeric data
  ...
  # D3_Nationality: using unique() function and standardizing input
...
# Cleaning the "OtherText" data fields
  ...
  # G2b_OtherText - no adjustments needed
  
  # E3_OtherText - no adjustments needed
...
################################################################################
  ...
  cd <- CleaningData                                                            # Copying cleaned Data and working with "cd" from now on, to distinguish steps but also because it's shorter :)
  cdPositive <- cd[cd$E1_Consider == 1, ]                                       # Cleaned Data only from People who'd consider investing into ELTIFs
...
### [2b] Isolated Analysis of each Variable ####################################
  ...
  # G1 / G2a
    # Histogram of Numbers of Asset Types
    ggplot(cd, aes(x = G2a_AssetTypes)) +
      geom_histogram(alpha = 1, position = "identity", binwidth = 1, color = "black")
  
    # Absolute & Relative Numbers of Asset Types
  ...
  # E3
    ...
    # Mentions only amongst E1_Consider = yes people
  ...
  # D1
    ...
    meanAge <- round(mean(cd$D1_Age, na.rm = TRUE), digits = 2)
      print(paste("Mean age of respondents: ", meanAge))
    medianAge <- median(cd$D1_Age, na.rm = TRUE)
      print(paste("Median age of respondents: ", medianAge))
...
################################################################################
  ...
  # G2a_AssetTypes -> E1_Consider
    Chi2_G2a_E1 <- table(cd$E1_Consider, cd$G2a_AssetTypes)                     # AssetTypes
      Chi2R_G2a_E1 <- chisq.test(Chi2_G2a_E1)
      print(Chi2R_G2a_E1)                                                       # p value = 0.04953 (*)
  ...
  # G2a_AssetTypes -> E2_Information
    Chi2_G2a_E2 <- table(cd$E2_Information, cd$G2a_AssetTypes)
      Chi2R_G2a_E2 <- chisq.test(Chi2_G2a_E2)
      print(Chi2R_G2a_E2)                                                       # p value = 0.01118 (*)
  ...
  # G3_Experience -> E1_Consider
    # Anova
    anovaR_G3_E1 <- aov(G3_Experience ~ E1_Consider, data = cd)
      summary(anovaR_G3_E1)                                                     # might be the wrong test!! p value = 0.000506 (***)
    # Chi2
  ...
  # G3_Experience and E2_Information 
    ...
    Chi2_G3_E2 <- table(cd$G3_Experience, cd$E2_Information)
      Chi2R_G3_E2 <- chisq.test(Chi2_G3_E2)
      print(Chi2R_G3_E2)                                                        # p-value = 0.4568     
    cor.test(cd$G3_Experience, cd$E2_Information)                               # p-value = 0.07229
  
  # G3_Experience and E4_Holding
    Chi2_G3_E4 <- table(cd$G3_Experience, cd$E4_Holding)
      Chi2R_G3_E4 <- chisq.test(Chi2_G3_E4)
      print(Chi2R_G3_E4)                                                        # p-value = 0.007571 (**)     
  ...
  # G4_RiskScore -> E1_Consider
    # Chi Squared
    Chi2_G4_E1 <- table(cd$E1_Consider, cd$G4_RiskScore)
      Chi2R_G4_E1 <- chisq.test(Chi2_G4_E1)
      print(Chi2R_G4_E1)                                                        # p value = 0.002922 (**)    
    # Anova
  ...
  # G4_RiskScore -> E2_Information
    # Chi Squared
    Chi2_G4_E2 <- table(cd$E2_Information, cd$G4_RiskScore)
      Chi2R_G4_E2 <- chisq.test(Chi2_G4_E2)
      print(Chi2R_G4_E2)                                                        # p value = 0.3109
    # Correlation
    CorTR_G4_E2 <- cor.test(cd$G4_RiskScore, cd$E2_Information)
      print (CorTR_G4_E2)                                                       # p value = 0.1221
    # Spearman
    spearmanR_G4_E2 <- cor.test(cd$G4_RiskScore, cd$E2_Information, method = "spearman")
      print(spearmanR_G4_E2)                                                    # p value = 0.07735; rho = - 0.105
    
  # G4_RiskScore -> E4_Holding
    anovaR_G4_E4 <- aov(G4_RiskScore ~ E4_Holding, data = cd)
      summary(anovaR_G4_E4)                                                     # p value = 0.00534 (**)
    cor.test(cd$G4_RiskScore, cd$E4_Holding)                                    # Correlation Coefficient = 0.1649
    spearmanR_G4_E4 <- cor.test(cd$E4_Holding, cd$G4_RiskScore, method = "spearman")
      print(spearmanR_G4_E4)                                                    # p value = 0.01252 (*); rho = 0.1480
  
  # G4_RiskScore -> E5_Withdrawal
    anovaR_G4_E5 <- aov(G4_RiskScore ~ E5_Withdrawal, data = cd)
      summary(anovaR_G4_E5)                                                     # p value = 0.0147 (*)
    cor.test(cd$G4_RiskScore, cd$E5_Withdrawal)                                 # Correlation Coefficient = 0.1445756
      
  # G5_TimeScore -> E1_Consider
    anovaR_G5_E1 <- aov(G5_TimeScore ~ E1_Consider, data = cd)
      summary(anovaR_G5_E1)                                                     # p value = 0.114
    Chi2_G5_E1 <- table(cd$E1_Consider, cd$G5_TimeScore)
      Chi2R_G5_E1 <- chisq.test(Chi2_G5_E1)
      print(Chi2R_G5_E1)                                                        # p value = 0.19.69  
      
  # G5_TimeScore and E2_Information
    spearmanR_G5_E2 <- cor.test(cd$G5_TimeScore, cd$E2_Information, method = "spearman")
      print(spearmanR_G5_E2)                                                    # p value = 0.002052 (**); rho = 0.1822
    cor.test(cd$G5_TimeScore, cd$E2_Information)                                # p value = 0.0345; Correlation Coefficient = 0.0345
  ...
  # E1_Consider -> E4_Holding
    anovaR_E1_E4 <- aov(E4_Holding ~ E1_Consider, data = cd)
      summary(anovaR_E1_E4)                                                     # p value = 0.00012 (***)
  ...
  # E1_Consider -> E5_Withdrawal
    anovaR_E1_E5 <- aov(E5_Withdrawal ~ E1_Consider, data = cd)
      summary(anovaR_E1_E5)                                                     # p value = 0.206
  ...
  # E1_Consider and E6_TrustScore
    ...
    multinom_E1_E6 <- multinom(E1_Consider ~ E6_TrustScore, data = cd)
      summary(multinom_E1_E6)
      # Extract the coefficients and standard errors
      summary_multinom <- summary(multinom_E1_E6)
      coefficients <- summary_multinom$coefficients
      standard_errors <- summary_multinom$standard.errors
      # Calculate z-values
  ...
  # E1_Consider and all Demographics
    anovaR_E1_D1 <- aov(E1_Consider ~ D1_Age, data = cd)                        # Age
  ...
  # E2_Information and E4_Holding
    spearmanR_E2_E4 <- cor.test(cd$E4_Holding, cd$E2_Information, method = "spearman")
      print(spearmanR_E2_E4)                                                    # p value = 0.4042
    Chi2_E2_E4 <- table(cd$E2_Information, cd$E4_Holding)
      Chi2R_E2_E4 <- chisq.test(Chi2_E2_E4)
      print(Chi2R_E2_E4)                                                        # p value = 0.4973
  
  # E2_Information and E6_TrustScore
    CorTR_E2_E6 <- cor.test(cd$E2_Information, cd$E6_TrustScore)
      print(CorTR_E2_E6)                                                        # p value = 0.44
    Chi2_E2_E6 <- table(cd$E2_Information, cd$E6_TrustScore)
      Chi2R_E2_E6 <- chisq.test(Chi2_E2_E6)
      print(Chi2R_E2_E6)                                                        # p value = 0.01212 (*)
    anovaR_E2_E6 <- aov(E2_Information ~ E6_TrustScore, data = cd)
      summary(anovaR_E2_E6)                                                     # p value = 0.44
  ...
  # E4_Holding and E5_Withdrawal
    ...
    Chi2_E4_E5 <- table(cd$E4_Holding, cd$E5_Withdrawal)
      Chi2R_E4_E5 <- chisq.test(Chi2_E4_E5)
      print(Chi2R_E4_E5)                                                        # p-value = 1.54e-08 (***)
    table(cd$E4_Holding, cd$E5_Withdrawal)
    round(prop.table(table(cd$E4_Holding, cd$E5_Withdrawal), 2), 4) 
  
### [3b] Other non-contributing Analyses #######################################   
